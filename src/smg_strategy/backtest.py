...
--------------------------------------------------------------------------------
策略累计收益率 (Total Return):       {self.total_return * 100:+.2f}%  (基准 SPY: {self.benchmark_total_return * 100:+.2f}%)
...
class BacktestEngine:
    ...
    def _compute_point_in_time_score(self, ticker: str, history_df: pd.DataFrame) -> float:
        ...
        mom_score = min(100.0, max(0.0, 50.0 + ret_20 * 200.0))

        # 2. 均线形态 (5日 / 20日 EMA)
        ema_5 = closes[-5:].mean()
        ema_20 = closes[-20:].mean()
        trend_score = 75.0 if ema_5 > ema_20 else 35.0

        # 3. 量能配合 (最近3日均量 vs 20日均量)
        ...
        vol_score = min(100.0, max(0.0, vol_ratio * 50.0))
        ...
        composite_score = 0.35 * mom_score + 0.25 * trend_score + 0.20 * vol_score + 0.20 * rsi
    ...
    def run(self) -> BacktestResult:
        ...
        trading_dates = [d.strftime("%Y-%m-%d") if hasattr(d, "strftime") else str(d)[:10] for d in bench_df.index]
        ...
        for t_idx, current_date in enumerate(trading_dates):
            ...
            for sym, pos in list(positions.items()):
                ...
                sym_low = float(sym_df["Low"].iloc[t_idx]) if "Low" in sym_df else sym_close

                # 更新历史峰值
                if sym_close > pos["peak_price"]:
                    pos["peak_price"] = sym_close

                # 止损判断
                # (a) GAES -6% 硬止损
                pnl_from_entry = (sym_low - pos["entry_price"]) / pos["entry_price"]
                if pnl_from_entry <= self.config.stop_loss:
                    exit_price = pos["entry_price"] * (1.0 + self.config.stop_loss) * (1.0 - slippage)
                    tickers_to_close.append((sym, exit_price, "STOP_LOSS"))
                    continue

                # (b) 5% 移动止损 (High-Water Mark Drawdown)
                dd_from_peak = (pos["peak_price"] - sym_low) / pos["peak_price"]
                if dd_from_peak >= self.config.trailing_stop and pos["peak_price"] > pos["entry_price"]:
                    exit_price = pos["peak_price"] * (1.0 - self.config.trailing_stop) * (1.0 - slippage)
            ...
            for sym, pos in positions.items():
                sym_df = self.price_data[sym]
                sym_close = float(sym_df["Close"].iloc[min(t_idx, len(sym_df) - 1)])
                portfolio_equity += pos["shares"] * sym_close

            # 3. 产生新的开仓决策 (需要至少 20 天历史进行指标计算)
            if t_idx >= 20:
                candidate_scores = []
                ...
                for sym, score in candidate_scores:
                    sym_df = self.price_data[sym]
                    sym_close = float(sym_df["Close"].iloc[t_idx])
                    buy_price = sym_close * (1.0 + slippage)

                    # SMG 约束：单票上限 20% 账户总权益
                    max_alloc = portfolio_equity * self.config.position_cap
