# SMG Quantitative Trading System (Stock Market Game)

[![Python 3.10+](https://img.shields.io/badge/python-3.10+-blue.svg)](https://www.python.org/)
[![Tests](https://img.shields.io/badge/tests-42%20passed-brightgreen.svg)]()
[![KaTeX](https://img.shields.io/badge/KaTeX-Math_Verified-00F0FF.svg)]()
[![UI](https://img.shields.io/badge/UI-Interactive_Web_Terminal-8B5CF6.svg)]()
[![Risk Shield](https://img.shields.io/badge/Risk_Shield-0.5s_Patrol-orange.svg)]()
[![License: MIT](https://img.shields.io/badge/license-MIT-green.svg)](LICENSE)

An institutional-grade automated quantitative trading, risk management, and historical backtesting system designed for **The Stock Market Game (SMG)**. Features multi-factor screening, 4-pillar quantitative synthesis (DCF with dynamic WACC, Broadie-Glasserman-Kou discrete-barrier corrected 100k-path Monte Carlo VaR, Union-Find correlation clustering, Kelly portfolio optimization), dynamic drawdown state machines, a 0.5s real-time risk patrol daemon, a full Point-in-Time historical backtesting engine with SPY benchmark comparison, and an interactive modern Web UI dashboard.

---

<p align="center">
  <img src="assets/system_architecture.svg" alt="SMG System Architecture" width="850"/>
</p>

---

## 🧮 Core Models & Mathematical Formulations

### 1. Multi-Stage Discounted Free Cash Flow (DCF) with Capital Structure WACC

```math
\text{WACC} = w_e \cdot (R_f + \beta \times \text{ERP}) + w_d \cdot K_d \cdot (1 - T_c)
```

| Parameter | Symbol | Baseline / Formula | Description |
| :--- | :---: | :--- | :--- |
| **Equity Weight** | $w_e$ | $\frac{E}{E+D}$ | Weight of equity in total enterprise capital |
| **Debt Weight** | $w_d$ | $\frac{D}{E+D}$ | Weight of debt in total enterprise capital |
| **Cost of Debt** | $K_d$ | $5.25\%$ | Pretax annual corporate borrowing rate |
| **Corporate Tax Rate** | $T_c$ | $21.0\%$ | Federal statutory corporate income tax |
| **Risk-Free Rate** | $R_f$ | $4.75\%$ | 10-Year US Treasury benchmark yield |
| **Equity Risk Premium** | $\text{ERP}$ | $5.50\%$ | Expected return spread of equities over risk-free rate |
| **WACC Bound** | $\text{WACC}_{\text{clamped}}$ | $[6.0\%, 15.0\%]$ | Dynamic boundary clamp to prevent cost-of-capital explosion |

#### Two-Stage Cash Flow Present Value Formulation
```math
\text{PV} = \sum_{t=1}^{5} \frac{\text{FCF}_0 \cdot (1+g_1)^t}{(1+\text{WACC})^t} + \frac{\text{FCF}_5 \cdot (1+g_{\text{term}})}{(\text{WACC} - g_{\text{term}})(1+\text{WACC})^5}
```

#### Per-Share Fair Value & Margin of Safety (MoS)
```math
\text{Fair Value} = \frac{\text{PV} - \text{Total Debt} + \text{Cash}}{\text{Shares Outstanding}}, \quad \text{MoS} = \frac{\text{Fair Value} - \text{Market Price}}{\text{Fair Value}}
```

---

### 2. Broadie-Glasserman-Kou (BGK) Barrier-Corrected Monte Carlo Simulation

#### Geometric Brownian Motion (GBM) Stochastics
```math
\ln S_{t+\Delta t} = \ln S_t + \left( \mu - \frac{1}{2}\sigma^2 \right)\Delta t + \sigma \sqrt{\Delta t} Z, \quad Z \sim \mathcal{N}(0, 1)
```

#### Discrete Barrier Boundary Shift (Broadie, Glasserman & Kou, 1997)
Standard daily closing observation introduces a systematic $\approx 6.37\%$ underestimation of intraday barrier crossings. To eliminate this discrete observation bias without requiring infinitesimally small simulation time steps, the absorption barrier is shifted outward by a boundary correction factor:

```math
\text{Effective Stop} = (1 + \text{Nominal Stop}) \cdot \exp\left( -\frac{\zeta(1/2)}{\sqrt{2\pi}} \cdot \sigma_{\text{step}} \right) - 1 \approx (1 + \text{Nominal Stop}) \cdot \exp(0.5826 \cdot \sigma_{\text{step}}) - 1
```

*Simulated with $M = 4$ intraday sub-steps across $N = 100,000$ geometric paths for continuous boundary fidelity.*

---

### 3. Kelly Criterion & Union-Find Transitive Correlation Clustering

#### Optimal Sizing Fraction
```math
f^* = p - \frac{1-p}{b}, \quad b = \frac{\bar{G}}{\bar{L}}
```
where $p$ represents the empirical probability of a winning trade, and $b$ is the win/loss profit ratio. The system deploys Half-Kelly sizing ($f^*/2$) to suppress volatility drag while strictly enforcing position concentration ceilings.

#### Disjoint-Set Union-Find (Transitive Risk Clustering)
Pairwise Pearson correlation matrix $R$ is calculated across daily log returns. Disjoint-Set Union (Union-Find) groups equities into connected risk components across transitive correlation edges:

```math
\text{If } \text{Corr}(A, B) > 0.70 \text{ and } \text{Corr}(B, C) > 0.70 \implies \{A, B, C\} \in \text{Same Risk Cluster}
```

- **Concentration Hard Guard**: If an existing holding occupies $\ge 20\%$ of portfolio equity, new allocations are strictly blocked ($0$ shares), ensuring full compliance with SMG competition rules.

---

### 4. High-Frequency Trailing Stop & Risk Defense Daemon

#### 5.0% Trailing Stop Loss (High-Water Mark Monitored)
```math
\text{Drawdown}_{\text{peak}} = \frac{P_{\text{max}} - P_t}{P_{\text{max}}} \ge 5.0\% \quad \text{and} \quad P_{\text{max}} > \text{Cost Basis}
```

#### GAES -6.0% Hard Disaster Floor
```math
P_t \le \text{Cost Basis} \times (1 - 0.06) \implies \text{Immediate Emergency Liquidation}
```

#### Exact Position Trimming Rule
When any position appreciates beyond $20.0\%$ total equity, the system computes the exact integer share count required to trim the exposure down to the $17.5\%$ safe buffer:

```math
\text{Shares to Sell} = \left\lfloor \frac{\text{Position Value} - (\text{Equity} \times 0.175)}{P_t} \right\rfloor + 1
```

---

## 🖥️ Interactive Quant Web Dashboard (UI)

The system features a zero-dependency, modern FinTech dark-mode interactive Web UI terminal (`smg_strategy/dashboard.py`):

```bash
# 1. Launch live interactive quant terminal in browser (default: http://127.0.0.1:8088)
PYTHONPATH=src python3 strategy_cli.py --dashboard

# 2. Specify a custom listening port
PYTHONPATH=src python3 strategy_cli.py --dashboard --port 9090

# 3. Export as a standalone, zero-dependency HTML dashboard for sharing or presentation
PYTHONPATH=src python3 strategy_cli.py --export-dashboard dashboard.html
```

### Dashboard Core Visual Capabilities
- **Portfolio Analytics & KPI Cards**: Real-time Total Equity, Buying Power, Cash Reserve, High-Water Mark, and Peak Drawdown.
- **Pillar 1: Multi-Stage DCF Valuation Gauges**: Dynamic fair-value bars vs current market price with Margin of Safety (MoS) badges.
- **Pillar 2: BGK Monte Carlo VaR Heatmap**: Simulated path distribution, empirical win probability, and boundary correction factor.
- **Pillar 3: Union-Find Correlation Matrix**: Cluster groupings and aggregate exposure guard recommendations.
- **Pillar 4: Half-Kelly Allocation & Cap Guard**: Real-time position weights vs 20% hard concentration limit.
- **0.5s Risk Patrol Live Table**: Real-time distance indicators to -5.0% HWM trailing stop and -6.0% hard floor.
- **Point-in-Time Historical Equity Curve**: Interactive vector charts benchmarking SMG strategy vs S&P 500 (SPY).

---

## 📁 Repository Structure

```
.
├── assets/
│   ├── system_architecture.svg       # Lossless vector architecture diagram
│   ├── system_architecture.png       # 3x Retina Ultra-HD architecture diagram
│   └── dashboard_preview.html        # Pre-rendered interactive Web UI preview
├── config/
│   └── example_snapshot.json         # Sanitized sample portfolio & market input
├── src/
│   └── smg_strategy/
│       ├── __init__.py
│       ├── dashboard.py              # Interactive Web UI terminal & HTML export engine
│       ├── config.py                 # Centralized parameter & threshold governance
│       ├── models.py                 # Immutable domain records (dataclasses)
│       ├── rules.py                  # Official SMG eligibility & rule constraints
│       ├── scoring.py                # Multi-factor long/short scoring formulas
│       ├── risk.py                   # Drawdown state machine & sizing limits
│       ├── report.py                 # Markdown review packet renderer
│       ├── scanner.py                # Dynamic market screener with yfinance
│       ├── quant_engine.py           # Chief Quant 4-model validation engine (BGK, Kelly, DCF, UF)
│       ├── hourly_quant_decision.py  # Hourly automated quant portfolio decision generator
│       ├── risk_patrol.py            # 0.5s real-time risk patrol daemon (HWM trailing stops)
│       ├── backtest.py               # Point-in-Time backtesting engine with SPY benchmark
│       ├── order_state_machine.py    # 9-state order state machine & risk gates
│       ├── ticker_locks.py           # Daily idempotent trade locks
│       └── pre_market_gate.py        # Pre-market broker reconciliation gate with audit trails
├── tests/
│   ├── test_models.py
│   ├── test_rules.py
│   ├── test_scoring.py
│   ├── test_risk.py
│   ├── test_report.py
│   ├── test_dashboard.py             # Unit tests for Web UI dashboard generation & export
│   ├── test_quant_fixes.py           # Unit tests for Kelly caps, Union-Find, BGK Monte Carlo, DCF
│   └── test_backtest.py              # Unit tests for backtest engine and performance metrics
├── strategy_cli.py                   # Unified CLI: review packet, Web dashboard & historical backtesting
└── README.md
```

---

## 🚀 Getting Started

### 1. Requirements
- Python 3.10+
- `numpy`, `scipy`, `pandas`, `yfinance`

```bash
pip install numpy scipy pandas yfinance
```

### 2. Run Comprehensive Unit Test Suite
```bash
PYTHONPATH=src python3 -m unittest discover -s tests -v
# Output: Ran 42 tests in 0.089s ... OK
```

### 3. Launch Interactive Web UI Dashboard
```bash
# Start local interactive server
PYTHONPATH=src python3 strategy_cli.py --dashboard

# Or generate portable HTML dashboard report
PYTHONPATH=src python3 strategy_cli.py --export-dashboard assets/dashboard_preview.html
```

### 4. Run Historical Point-in-Time Backtesting
Test quantitative strategies against historical data with automated SPY benchmark comparison, next-day open execution ($t+1$ Open), and ASCII equity curves:

```bash
# Quick backtest on top tech leaders
PYTHONPATH=src python3 strategy_cli.py --backtest --tickers "AAPL,NVDA,MSFT,AMZN" --start "2024-01-01" --end "2024-06-30"

# Backtest with JSON output export
PYTHONPATH=src python3 strategy_cli.py --backtest --tickers "AAPL,NVDA,GOOGL" --start "2024-01-01" --end "2024-04-01" --json-output backtest_results.json
```

**Key Backtest Capabilities & Performance Metrics**:
- **Point-in-Time Execution**: Eliminates lookahead bias by scoring on day $t$ close and filling orders at day $t+1$ Open with slippage.
- **Realistic Slippage & Gap-Down Stops**: Factors in overnight gap-down risks (`min(target_stop, open)`).
- **Walk-Forward Overfitting Analysis**: Built-in `run_walk_forward()` utility splits historical regimes to measure out-of-sample decay.
- **Metrics Reported**: Excess Return vs SPY, Total Return, CAGR, Annualized Volatility, Sharpe Ratio ($R_f = 4.75\%$), Maximum Drawdown, Calmar Ratio, Jensen's Alpha, Beta, Win Rate, and Profit Factor.

### 5. Generate Strategy Review Packet
```bash
PYTHONPATH=src python3 strategy_cli.py config/example_snapshot.json --output latest_report.md
```

### 6. Run Chief Quant 4-Model Validation Engine
```bash
PYTHONPATH=src python3 src/smg_strategy/quant_engine.py
```

### 7. Run High-Frequency Risk Patrol Daemon
```bash
PYTHONPATH=src python3 src/smg_strategy/risk_patrol.py
```

---

## ⚖️ Disclaimer

This codebase is developed strictly for educational and research purposes within the virtual simulation of **The Stock Market Game (SMG)**. It does not constitute financial, investment, or legal advice.
